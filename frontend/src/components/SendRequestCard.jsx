...
const SendRequestCard = ({ friend, outgoingFriendReqs, sendFriendRequest }) => {
  ...
  return (
    <div className="bg-gray-800 w-90 flex-shrink-0 rounded-3xl p-5 border border-gray-700 hover:border-emerald-500 transition-all duration-300 overflow-hidden">
      ...
      {/* Action Buttons */}
      {outgoingFriendReqs.includes(friend._id) ? (
        <div>
          <button className="flex-1 min-w-[120px] px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg font-medium text-sm cursor-pointer whitespace-nowrap">
            Pending Request
          </button>
        </div>
      ) : (
        <div className="flex space-x-2 pt-2 flex-wrap gap-2">
          <button
            ...
            className="flex-1 min-w-[120px] px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium text-sm cursor-pointer whitespace-nowrap"
          >
            Send Request
          </button>
        </div>
