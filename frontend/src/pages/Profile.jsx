...
import { editProfile } from '../utils/apiPaths';
...
const Profile = () => {
  ...
  const [userDetails, setUserDetails] = useState({
    bio: authUser?.bio || '',
    skills: (authUser?.skills || []).filter(
      (skill) => skill && skill.toLowerCase() !== 'ok'
    ),
  ...
  // Sync userDetails when authUser changes
  useEffect(() => {
    if (authUser) {
      setUserDetails({
        bio: authUser.bio || '',
  ...
  const { mutateAsync: saveProfile, isPending, error } = useMutation({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Error handled by mutation onSuccess/error
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
      <div className="w-full max-w-2xl bg-gray-800 rounded-xl shadow-lg p-4">
        <h1 className="text-3xl font-bold text-center text-white mb-8">Edit Your Profile</h1>
        <form onSubmit={handleSubmit} className="space-y-2">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">Type a skill and press Enter or Add. Skills are shown as tags below.</p>
          </div>
